...
float Vector_Norm(sVector3_t a){
	return sqrt(a.vector[0]*a.vector[0] + a.vector[1]*a.vector[1] + a.vector[2]*a.vector[2]);
}
...
void Vector_Rotate(sVector3_t* a, float roll, float pitch, float yaw){
	float x = a->vector[0];
	float y = a->vector[1];
	float z = a->vector[2];
