/**
  ...
  */
          ...
          uint8_t				*p_is_alive)
  {
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Checking if TOF is alive...");
      #endif
  
      status |= WrByte(&(p_dev->platform), 0x7fff, 0x00);
      status |= RdByte(&(p_dev->platform), 0x0000, &(p_dev->device_id));
      status |= RdByte(&(p_dev->platform), 0x0001, &(p_dev->revision_id));
      ...
      if (p_dev->module_type != (uint8_t)VL53LMZ_MODULE_TYPE_UNKNOWN){
          #ifdef DEBUG_TOF_API
              ESP_LOGI("TOF_API", "TOF alive");
          #endif
          *p_is_alive = 1;
      }
      else{
          #ifdef DEBUG_TOF_API
              ESP_LOGI("TOF_API", "TOF NOT alive");
          #endif
          *p_is_alive = 0;
      }
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Device ID: %d\tRevision ID: %d", p_dev->device_id, p_dev->revision_id);
      #endif
          ...
          VL53LMZ_Configuration		*p_dev)
  {
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Waiting device to boot...");
      #endif
      status |= WrByte(&(p_dev->platform), 0x7fff, 0x00);
      ...
      /* Enable FW access */
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Enable FW access");
      #endif
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Enable host access to G01");
      #endif
      status |= RdByte(&(p_dev->platform), 0x7fff, &tmp);
      ...
      /* Power ON status */
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Power on status");
      #endif
      status |= WrByte(&(p_dev->platform), 0x7fff, 0x00);
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Download FW into VL53LMZ");
      #endif
  
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Sending first part...");
      #endif
  
      status |= WrByte(&(p_dev->platform), 0x7fff, 0x09);
      status |= WrMulti(&(p_dev->platform),0x0000,
          (uint8_t*) &VL53LMZ_FIRMWARE[0], 0x8000);
  
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Sending second part...");
      #endif
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Sending third part...");
      #endif
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Check if FW correctly downloaded");
      #endif
      if ( p_dev->revision_id == (uint8_t)REVISION_L8 ) {
          status |= WrByte(&(p_dev->platform), 0x7fff, 0x01);
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Reset MCU and wait boot");
      #endif
      status |= WrByte(&(p_dev->platform), 0x7FFF, 0x00);
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Get offset NVM data and store them into offset buffer");
      #endif
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Set default Xtalk shape");
      #endif
      ...
      tmp = (uint8_t)1;
      status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
              VL53LMZ_DCI_GLARE_FILTER_CFG, 40, (uint8_t*)&tmp, 1, 0x26);
      status |= vl53lmz_dci_replace_data(p_dev, p_dev->temp_buffer,
              VL53LMZ_DCI_GLARE_FILTER_CFG, 40, (uint8_t*)&tmp, 1, 0x25);
          ...
          uint16_t				i2c_address)
  {
      ...
      #ifdef DEBUG_TOF_API
          ESP_LOGI("TOF_API", "Changing TOF address to %x", i2c_address);
      #endif
