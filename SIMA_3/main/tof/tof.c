 ...
 */
 ...
 void tof_calculate_distances_interrupt(VL53LMZ_Object* tof, VL53LMZ_Result_t *data){
     ...
     else tof->interrupt_left_zone = 0;
     ESP_LOGW("Tof", "L1: %lu L2: %lu", data->ZoneResult[TOF_LEFT_ZONE_1].Distance, data->ZoneResult[TOF_LEFT_ZONE_2].Distance);
     ESP_LOGI("Status", "L1: %lu L2: %lu", data->ZoneResult[TOF_LEFT_ZONE_1].Status, data->ZoneResult[TOF_LEFT_ZONE_2].Status);
     ...
     else tof->interrupt_right_zone = 0;
     ESP_LOGW("Tof", "R1: %lu R2: %lu", data->ZoneResult[TOF_RIGHT_ZONE_1].Distance, data->ZoneResult[TOF_RIGHT_ZONE_2].Distance);
           ESP_LOGI("Status", "R1: %lu R2: %lu", data->ZoneResult[TOF_RIGHT_ZONE_1].Status, data->ZoneResult[TOF_RIGHT_ZONE_2].Status);
     ...
     }else if(data->ZoneResult[TOF_CENTER_ZONE_1+1].Status == 0 && data->ZoneResult[TOF_CENTER_ZONE_1+1].Distance <= TOF_INTERRUPT_CENTER)
     {
         tof->interrupt_center_zone = 1;
     ...
     else tof->interrupt_center_zone = 0;
     ESP_LOGW("Tof", "C1: %lu C2: %lu C3: %lu C4: %lu", data->ZoneResult[TOF_CENTER_ZONE_1].Distance, data->ZoneResult[TOF_CENTER_ZONE_1+1].Distance, data->ZoneResult[TOF_CENTER_ZONE_2].Distance, data->ZoneResult[TOF_CENTER_ZONE_2+1].Distance);
          ESP_LOGI("Status", "C1: %lu C2: %lu C3: %lu C4: %lu", data->ZoneResult[TOF_CENTER_ZONE_1].Status, data->ZoneResult[TOF_CENTER_ZONE_1+1].Status, data->ZoneResult[TOF_CENTER_ZONE_2].Status, data->ZoneResult[TOF_CENTER_ZONE_2+1].Status);
...
bool check_tof(VL53LMZ_Result_t data, VL53LMZ_Interrupt_Zone zone, Socket rpi_socket)
{
    ...
    if(zone.interrupt_center){
        ...
        //socket_send(&rpi_socket);
        //ESP_LOGW("TOF", "L: %d", zone.interrupt_left);
        return true;
    }
    ...
    if(zone.interrupt_right){
        ...
        //socket_send(&rpi_socket);
        //ESP_LOGW("TOF", "R: %d", zone.interrupt_right);
        return true;
    }
    ...
    if(zone.interrupt_left){
        ...
        //socket_send(&rpi_socket);
        //ESP_LOGW("TOF", "C: %d", zone.interrupt_center);
        return true;
    }
    ESP_LOGW("TOF", "L: %d C: %d R: %d", zone.interrupt_left, zone.interrupt_center, zone.interrupt_right);
