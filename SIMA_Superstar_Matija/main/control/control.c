...
const target_zone_t zone1 = { 850.0, 1250.0, -1400.0, -1500.0 };
const target_zone_t zone2 = { 1250.0, 1700.0, -1250.0, 1500.0 };   //{ 1250.0, 1700.0, -1250.0, -1500.0 }
const target_zone_t zone3 = { 1750.0, 2100.0, -1400.0, -1500.0 };
...
void speed_control(float target_x, float target_y, float target_speed, position_t *robot)
{
    ...
    float Kp_lin = 1.25; //2.1
    float Ki_lin = 0.0;
    float Kd_lin = 3.5;   //250.1
...
{
    static float kp = 5.6;    //7.75 za mali (8.2), 7.7 za veliki (~8.05)
    static float ki = 0.00001; //1.9 za mali(1.75), 1.6 za veliki (~1.54)
    static float kd = 0.5; //0.52 za mali, 0.32 za veliki 0.001
    ...
    // if(u < last_u_left - 0.1) u = last_u_left - 0.1;
    // last_u_left = u;
    ESP_LOGW(TAG_Control, "UL: %f, Target_L: %f", u, target);
    if (u < 0.0) u = 0.0;
...
{
    static float kp = 5.5; //8.0 za veliki (~7.94), 7.9 za mali (7.6)
    static float ki = 0.00002; //1.75 (1.4)
    static float kd = 0.0;  //0.001
    ...
    // if(u < last_u_right - 0.1) u = last_u_right - 0.1;
    // last_u_right = u;
    ESP_LOGW(TAG_Control, "UR: %f, Target_R: %f", u, target);
    if (u < 0.0) u = 0.0;
    else if (u > 95.0) u = 95.0;
    //motor_forward(motor, u);
    return u;
...
void rotate_right(float target_angle, encoder_info_queue_t *enc, position_t *robot, motor_t right_motor, motor_t left_motor)
{
    ...
    if (angle_count_deg < target_angle)
    {
        ...
        rotate_control(angle_count_deg, target_angle, 3.5);
        float speed_r = pid_r(right_motor, v_ang_right);
        float speed_l = pid_l(left_motor, v_ang_left);
        ESP_LOGW("Log", "SR: %f SL: %f", speed_r, speed_l);
...
void rotate_left(float target_angle, encoder_info_queue_t *enc, position_t *robot, motor_t right_motor, motor_t left_motor)
{
    ...
    if (angle_count_deg < target_angle)
    {
        ...
        angle_count_deg = angle_count * 180/PI;
        rotate_control(angle_count_deg, target_angle, 7.0);
        float speed_r = pid_r(right_motor, v_ang_right);
        float speed_l = pid_l(left_motor, v_ang_left);
