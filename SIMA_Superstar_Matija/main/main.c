...
void app_main(void)
{
	...
	if (!queue)
	{
        ...
        coord_t coords[5];
    #else
        coord_t coords[] = { { 2875.0, 1905.0, 3.14, 0.0}, /*{ 500.0, -395.0, 0.0, 50.0 }*/ { 1825.0, 1855.0, 0.0, 50.0 },/*, { 1275.0, -250.0, 0.0, 30.0 },*/ { 1700.0, 1300.0, 0.0, 30.0 } };
    ...
    #endif
        // coord_t coords[] = { {125.0, 276.0, 0.0, 0.0}, { 300.0, 276.0, 0.0, 50.0 }, { 900.0, 600.0, 0.0, 50.0 }, { 1300.0, 570.0, 0.0, 50.0 } };    //125.0, 395.0, 1000, 600
        // data_size = sizeof(coords)/16;
    ...
    while (1)
    {
        ...
        switch(robot_states)
        {
            ...
            break;
            case ROBOT_MOVE:
                go_to_xy(coords[i].x, coords[i].y, coords[i].speed, &encoder, &robot, right_motor, left_motor, pcnt1, pcnt2, rpi_socket);
            ...
            case ROBOT_AVOID:
                ...
                /*rotate_right(45.0, &encoder, &robot, right_motor, left_motor);
                if (robot_rotated) robot_states = ROBOT_STOPPED;
                if (!avoiding)
                {
                    calc_pos(&encoder, &robot);                  
                    if (check_zone(robot, target_zone))
                    {
                        in_position = true;
                        robot_states = ROBOT_STOPPED;
                    }
                    ...
                    {
                        avoiding = true;
                        ESP_LOGI("Avoid", "Setuje avoiding");
                    }
                }
                else
                {
                    avoid_obstacle(&robot, &encoder, right_motor, left_motor, pcnt1, pcnt2, data, zone, rpi_socket);
                }
                if (avoidance_complete) robot_states = ROBOT_MOVE;*/
            break;
            case ROBOT_STOPPED:
                motors_stop(right_motor, left_motor);
                robot_stopped = true;
        ...
        #ifdef DETECTION
            if (check_tof(data, zone, rpi_socket) && temp == 1)
            {
                //sprintf(&rpi_socket.tx_buff, "Detektovao");
                //socket_send(&rpi_socket);
                store_temp = temp;
            ...
            if (!check_tof(data, zone, rpi_socket) && robot_stopped && !in_position)
            {
                temp = store_temp;
                robot_stopped = false;
