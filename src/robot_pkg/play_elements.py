...
class Area:
    ...
    def get_all_areas(cls):
        ...
        '''
        return [(name, value) for name, value in vars(cls).items() if isinstance(value, ElementPosition)]

    @classmethod
    def get_all_visited_areas(cls):
        '''
        Gets all VISITED areas on the field. If a color is passed, returns visited fields of that color. If nothing is passed, returns all fields.
        ...
        '''
        areas = cls.get_all_areas()

        visited = [value for name, value in areas if isinstance(
            value, ElementPosition) and value.visited]
...
class MaterialStack:
    ...
    def get_closest_stack(cls, curr_x, curr_y):
        ...
        available_stacks = [value for name,
                            value in cls.get_all_unvisited_stacks()]
        if not len(available_stacks):
            return None

        return min(available_stacks, key=lambda p: p.distance_to(curr_x, curr_y))
