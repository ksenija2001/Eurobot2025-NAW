...
class FieldVisualizer:
    def __init__(self):
        ...
        self.animation = FuncAnimation(self.fig, self.update_positions, interval=100)
    ...
    def _setup_field(self):
        ...
        self.ax.grid(True)
        self.ax.set_title('Robot Soccer Field')
    ...
    def _draw_areas(self):
        ...
        for area in Area.get_all_areas():
            color = 'yellow' if 'YELLOW' in str(area) else 'blue'
    ...
    def _update_animation(self, frame):
        ...
        self.update_positions(
            ...
            opponent_x=Lidar.opponent.x if hasattr(Lidar, 'opponent') else None,
            opponent_y=Lidar.opponent.y if hasattr(Lidar, 'opponent') else None,
            opponent_theta=Lidar.opponent.theta if hasattr(Lidar, 'opponent') else None
        )
    ...
    def update_positions(self, robot_x, robot_y, robot_theta, 
        ...
        if opponent_x is not None:
            ...
            self.ax.plot(*zip(*self.opponent_trail), 'r-', alpha=0.3)  # Red trail
        
        # Add text labels
        self.ax.text(robot_x, robot_y + 150, 'Robot', ha='center', va='center', color='blue')
        if opponent_x is not None:
            self.ax.text(opponent_x, opponent_y + 150, 'Opponent', ha='center', va='center', color='red')
